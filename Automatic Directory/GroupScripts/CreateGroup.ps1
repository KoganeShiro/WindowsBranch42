<#
.SYNOPSIS
    Creates an Active Directory group in a specified Organizational Unit (OU) with configurable parameters and input validation, supporting both command-line and GUI input.

.DESCRIPTION
    This script provides a configurable and extensible framework for creating Active Directory groups. 
    It defines all parameters and their validation rules in a configuration section, supports both command-line and GUI input, 
    and performs comprehensive validation before executing the main logic. The script ensures that the specified OU exists, 
    the group does not already exist, and that all inputs meet the defined requirements. 
    It requires administrative privileges and the Active Directory module.
...
.PARAMETER OrganisationUnitToJoin
    The name of the Organizational Unit (OU) where the group will be created.
    - Type: String
    - Required: Yes
    - Validation: 1-20 characters, no leading/trailing spaces, no invalid special characters.
...
.INPUTS
    Accepts parameters via command-line arguments or through a dynamically generated Windows Forms GUI if required parameters are missing.

.OUTPUTS
    Writes status and error messages to the host. Creates a new Active Directory group if all validations pass.

.NOTES
    - Requires the Active Directory module.
    - Must be run with Administrator privileges.
    - Designed for extensibility: parameters and validation can be modified in the configuration section.
    - GUI input is provided for missing or incomplete parameters.

.EXAMPLE
    .\CreateGroup.ps1 -GroupName "MyGroup" -OrganisationUnitToJoin "MyOU" -GroupScope "Global" -Description "Test group"

    Creates a new group named "MyGroup" in the "MyOU" organizational unit with a global scope and the specified description.
