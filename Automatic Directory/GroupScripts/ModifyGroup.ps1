<#
.SYNOPSIS
    Modifies an attribute of an Active Directory group with dynamic parameter handling and GUI support.

.DESCRIPTION
    This script allows modification of a specified attribute for an Active Directory group. 
    Parameters are defined in a configuration section, enabling dynamic validation, GUI input, and command-line argument parsing.
    The script ensures required parameters are provided, validates input, and provides a Windows Forms GUI if any required parameters are missing.
    It checks for administrator privileges, validates group existence, and updates the specified attribute using Set-ADGroup.
    Comprehensive error handling and user feedback are included.
...
.PARAMETER AttributeName
    Name of the attribute to be edited.
    - Type: string
    - Required: Yes
...
.INPUTS
    Accepts parameters via command-line arguments or GUI input.

.OUTPUTS
    Writes status and error messages to the host.

.EXAMPLE
    .\ModifyGroup.ps1 -GroupName "HRTeam" -AttributeName "Description" -DesiredValue "Human Resources Group"
    Modifies the 'Description' attribute of the 'HRTeam' group.

.NOTES
    - Requires Active Directory module.
    - Must be run with Administrator privileges.
    - Designed for extensibility with additional parameters and validation logic.
#>
...
$CONFIG = @{
    Parameters = @(
          @{
            ...
            Type = "string"
            Required = $true
            Description = "Name of the group to be modified"
            Validation = @{
                MinLength = 1
                MaxLength = 20
                Pattern = "^[a-zA-Z0-9._-]+$"
    ...
    MainLogic = {
        ...
        # Validate group exists before attempting to edit attribute, this prevents errors that would occur if we try to edit an attribute of a non-existent group, and it provides clear feedback to the user about the issue
        ...
        # Build the parameters hashtable for Set-ADGroup, using a hashtable allows us to easily manage and pass multiple parameters to the cmdlet, and it keeps the code cleaner and more maintainable
        $EditParams = @{            
            Identity=$groupName
...
#            }          
        ...
        # Edit the group attribute with error handling, using a try-catch block allows us to handle any exceptions that may occur during attribute editing gracefully, and it provides feedback on what went wrong if the operation fails
        try {        
            Set-ADGroup @EditParams            
            Write-Host "Attribute '$attributeName' for group '$groupName' edited successfully, set to '$desiredValue'." -ForegroundColor Green            
        }
        catch {
        ...
        try{
         ...
         #   $attributeName=$desiredValue
            ...
            Write-Host "Error editing group attribute: $_" -ForegroundColor Red
            exit 1
        }        
        }
    }
