<#
.SYNOPSIS
    Interactive script to retrieve Active Directory group information with GUI and validation.

.DESCRIPTION
    This script allows users to query information about Active Directory groups. 
    It supports both command-line and GUI input for parameters, with dynamic validation based on configuration.
    The script ensures required parameters are provided, validates input, and retrieves specified group attributes from Active Directory.
...
.PARAMETER Attributes
    List of group attributes to retrieve.
    - Required: No
    - Type: String array
    - Validation: Minimum 0 items.
    - If not specified, all attributes are retrieved.

.INPUTS
    Parameters can be provided via command-line arguments or through a Windows Forms GUI if required parameters are missing.

.OUTPUTS
    Displays group information in the console.
    Outputs selected attributes or all attributes of the specified group.

.NOTES
    - Requires the ActiveDirectory module.
    - Must be run as Administrator.
    - Designed for extensibility: parameters and main logic are defined in a configuration section.

.EXAMPLE
    .\ReadGroupInformation.ps1 -GroupName "Domain Admins" -Attributes "Description,Members"

    Retrieves the Description and Members attributes for the "Domain Admins" group.

.EXAMPLE
    .\ReadGroupInformation.ps1

    Launches the GUI for input if required parameters are not provided.
...
$CONFIG = @{
    Parameters = @(
          @{
            ...
            Type = "string"
            Required = $true
            Description = "Name of the group to query"
            Validation = @{
                MinLength = 1
                MaxLength = 20
                Pattern = "^[a-zA-Z0-9._-]+$"
    ...
    MainLogic = {
        ...
        # Validate group exists before attempting to retrieve information, this prevents errors that would occur if we try to access information of a non-existent group, and it provides clear feedback to the user about the issue
        ...
        # Retrieve group properties, if specific properties are provided, retrieve only those, otherwise retrieve all properties, this allows for flexibility in retrieving group information based on the user's needs, and it can improve performance by only retrieving necessary properties
        if ($attributes -and $attributes.Count -gt 0) {
            Write-Host "Retrieving properties for group '$groupName': $($attributes -join ', ')" -ForegroundColor Cyan
            get-adgroup -Identity $groupName -Properties * | Select-Object -Property $attributes
        } else {
            Write-Host "Retrieving all properties for group '$groupName'..." -ForegroundColor Cyan
