<#
.SYNOPSIS
    Adds a specified user to a specified Active Directory group with GUI and command-line support.

.DESCRIPTION
    This script allows administrators to add a user to an Active Directory group. 
    It supports both command-line arguments and a graphical user interface (GUI) for input.
    The script dynamically handles parameters based on a configuration section, validates input, 
    checks for the existence of the specified user and group, and performs the addition with error handling.
    The script must be run with Administrator privileges.

.PARAMETER GroupName
    The name of the Active Directory group to which the user will be added.
    - Type: String
    - Required: Yes
    - Validation: 1-20 characters, alphanumeric, dot, underscore, or hyphen.

.PARAMETER UserName
    The name of the user to add to the specified group.
    - Type: String
    - Required: Yes
    - Validation: 1-20 characters, alphanumeric, dot, underscore, or hyphen.
...
.EXAMPLE
    .\AddUserToGroup.ps1 -GroupName "HRTeam" -UserName "jdoe"
    Adds user 'jdoe' to the 'HRTeam' group.
...
.NOTES
    ...
    - Designed for extensibility via the configuration section.
...
$CONFIG = @{
    ...
    MainLogic = {
        ...
        # Validate group exists before attempting to add user to group, this prevents errors that would occur if we try to add a user to a non-existent group, and it provides clear feedback to the user about the issue
        ...
        # Validate user exists before attempting to add to group, this prevents errors that would occur if we try to add a non-existent user to the group, and it provides clear feedback to the user about the issue
        ...
        # Add the user to the group with error handling, using a try-catch block allows us to handle any exceptions that may occur during the addition process gracefully, and it provides feedback on what went wrong if the operation fails
        try {        
