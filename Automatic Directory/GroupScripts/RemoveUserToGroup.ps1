<#
.SYNOPSIS
    Removes a specified user from a specified Active Directory group with GUI and command-line support.

.DESCRIPTION
    This script allows administrators to remove a user from an Active Directory group. 
    It supports both command-line arguments and a Windows Forms GUI for parameter input.
    The script validates input parameters, checks for the existence of the group and user, 
    ensures the user is a member of the group, and then removes the user from the group.
    All operations are performed with error handling and user-friendly feedback.

.PARAMETER GroupName
    The name of the Active Directory group from which the user will be removed.
    Must be 1-20 characters long and contain only letters, numbers, dots, underscores, or hyphens.

.PARAMETER UserName
    The name of the user to remove from the specified group.
    Must be 1-20 characters long and contain only letters, numbers, dots, underscores, or hyphens.

.INPUTS
    Accepts parameters via command-line arguments or through a GUI prompt if required parameters are missing.

.OUTPUTS
    Writes status and error messages to the host. Exits with code 0 on success, 1 on error, or 0 if cancelled.

.EXAMPLE
    .\RemoveUserToGroup.ps1 -GroupName "HRTeam" -UserName "jdoe"

    Removes user 'jdoe' from group 'HRTeam' if all validations pass.

.EXAMPLE
    .\RemoveUserToGroup.ps1

    Launches a GUI to prompt for GroupName and UserName, then removes the user from the group.

.NOTES
    - Requires the Active Directory module.
    - Must be run with Administrator privileges.
    - Designed for maintainability and extensibility via the $CONFIG hashtable.
...
$CONFIG = @{
    ...
    MainLogic = {
        ...
        # Validate group exists before attempting to remove user from group, this prevents errors that would occur if we try to remove a user from a non-existent group, and it provides clear feedback to the user about the issue
        ...
        # Validate user exists before attempting to remove from group, this prevents errors that would occur if we try to remove a non-existent user from the group, and it provides clear feedback to the user about the issue
        $userExists = Get-ADUser -Filter "Name -eq '$userName'" -ErrorAction Stop            
        ...
        # Validate user is a member of the group before attempting to remove, this prevents errors that would occur if we try to remove a user who is not a member of the group, and it provides clear feedback to the user about the issue
        $isMember = Get-ADGroupMember -Identity $groupName -Recursive | Where-Object { $_.SamAccountName -eq $userName }
        if ($isMember) {
            Write-Host "User '$userName' is a member of group '$groupName'." -ForegroundColor Green
        } else {
            Write-Host "User '$userName' is not a member of group '$groupName', exiting." -ForegroundColor Red
            exit 1
        }

        # Build the parameters hashtable for Remove-ADGroupMember, using a hashtable allows us to easily manage and pass multiple parameters to the cmdlet, and it keeps the code cleaner and more maintainable
        $Params = @{            
            Identity=$groupName
            Members = $userName
            Confirm = $false
        ...
        # Remove the user from the group with error handling, using a try-catch block allows us to handle any exceptions that may occur during the removal process gracefully, and it provides feedback on what went wrong if the operation fails
        try {        
