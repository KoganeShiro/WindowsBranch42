<#
.SYNOPSIS
    Lists users in a specified Active Directory group with GUI and command-line support.

.DESCRIPTION
    This script allows you to list all user members of a specified Active Directory group.
    ...
    and ensures the script is run with Administrator privileges. It provides clear feedback and error handling.

.PARAMETER GroupName
    The name of the Active Directory group whose user members will be listed.
    - Type: String
    - Required: Yes
    - Validation: 1-20 characters, alphanumeric, dot, underscore, or hyphen.

.EXAMPLE
    .\ListUserInGroup.ps1 -GroupName "DomainAdmins"
    Lists all user members of the "DomainAdmins" group.

.EXAMPLE
    .\ListUserInGroup.ps1
    Launches the GUI for input if required parameters are missing.

.NOTES
    - Requires the Active Directory module.
    - Must be run with Administrator privileges.
    - Only user objects are listed (not computers or groups).
    - Provides both GUI and command-line parameter input.
...
$CONFIG = @{
    Parameters = @(
          @{
            ...
            Type = "string"
            Required = $true
            Description = "Name of the group to list members from Active Directory."
            Validation = @{
                MinLength = 1
                MaxLength = 20
                Pattern = "^[a-zA-Z0-9._-]+$"
    ...
    MainLogic = {
        ...
        # Validate group exists before attempting to list members, this prevents errors that would occur if we try to list members of a non-existent group, and it provides clear feedback to the user about the issue
        ...
        # Build the parameters hashtable for Get-ADGroupMember, using a hashtable allows us to easily manage and pass multiple parameters to the cmdlet, and it keeps the code cleaner and more maintainable
        $Params = @{            
            Identity=$groupName                        
            ErrorAction = "Stop"            
        }
        
        # List the group members with error handling, using a try-catch block allows us to handle any exceptions that may occur during member listing gracefully, and it provides feedback on what went wrong if the operation fails
        try {        
            Get-ADGroupMember @Params | Select-Object Name, SamAccountName, ObjectClass | Where-Object { $_.ObjectClass -eq "user" }
            # Get-ADGroupMember @Params 
            Write-Host "Members of group '$groupName' listed successfully." -ForegroundColor Green            
        }
        catch {
            Write-Host "Error listing group members: $_" -ForegroundColor Red
            exit 1
        }        
    }
}
