<#
.SYNOPSIS
Retrieves user account information from Active Directory based on specified filter and attributes.

.DESCRIPTION
This script provides a flexible framework for querying Active Directory user accounts. It features:
- Dynamic parameter configuration system
- GUI input form for missing required parameters
- Comprehensive parameter validation
- Attribute selection and filtering capabilities
- Support for both command-line and interactive execution

The script uses a configuration-driven approach where all parameters are defined in the $CONFIG hashtable, allowing easy modification without changing core logic.

.PARAMETER AccountNameFilter
Filter string for user accounts to retrieve. Supports wildcards (e.g., 'John*' retrieves all users with names starting with 'John'). 
Use '*' to retrieve all users.
Type: String
Required: Yes
Validation: 1-100 characters

.PARAMETER Attributes
List of specific Active Directory attributes to retrieve (one per line).
...
.\ReadDataBaseInformation.ps1 -AccountNameFilter "John*" -Attributes "Name,SamAccountName,UserPrincipalName"

.EXAMPLE
# Run interactively - GUI will appear if parameters are not provided
.\ReadDataBaseInformation.ps1

.NOTES
- Requires Administrator privileges
- Requires Active Directory PowerShell module
- Must be run on a domain-joined computer with access to Active Directory
- Multiline attribute input in GUI: one attribute per line

.FUNCTIONALITY
Main Logic:
1. Validates Administrator privileges
2. Imports Active Directory module
3. Retrieves users matching the specified SamAccountName filter
4. Selects and displays specified attributes or all attributes
5. Returns formatted user information
...
$CONFIG = @{
    Parameters = @(
          @{
            ...
            Type = "string"
            Required = $true
            Description = "Filter for user accounts to retrieve (e.g., 'John*' to retrieve all users with names starting with 'John'), * for all users"
            Validation = @{
                MinLength = 1
                MaxLength = 100                
            }
    ...
    MainLogic = {
        param($params)
        $accountNameFilter = $params["AccountNameFilter"]
        ...
        # Get users based on the provided filter, using Get-ADUser with a filter allows us to efficiently retrieve only the user accounts that match the specified criteria, and it can improve performance by reducing the number of objects retrieved from Active Directory
        Write-Host "Retrieving users with filter: '$accountNameFilter'..." -ForegroundColor Cyan
        $users = Get-ADUser -Filter "SamAccountName -like '$accountNameFilter'" -Properties *
        ...
        if ($attributes -and $attributes.Count -gt 0) {
            Write-Host "Retrieving attributes for all users: $($attributes -join ', ')" -ForegroundColor Cyan
            $users | Select-Object -Property $attributes
        } else {
            Write-Host "Retrieving all attributes for all users..." -ForegroundColor Cyan
            $users
        }
    }
}
